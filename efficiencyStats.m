...
% two-way ANOVA
% [p,t,stats] = anovan(allst,{condition portion},'model','interaction','varnames',{'Condition';'Dive Portion'});
...
% [nanmean(eta_low(:,1)) nanstd(eta_low(:,1))] % low descent
% [nanmean(eta_high(:,1)) nanstd(eta_high(:,1))] % high descent
% [mean(eta_low(:,2)) std(eta_low(:,2))] % low drag ascent
% [nanmean(eta_high(:,2)) nanstd(eta_high(:,2))] % high drag ascent

[nanmean(alleta(portion == 1)) nanstd(alleta(portion == 1))]; % all ascents
[nanmean(alleta(portion == -1)) nanstd(alleta(portion == -1))]; % all descents
